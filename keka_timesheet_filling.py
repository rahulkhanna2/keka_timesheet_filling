...
for index, row in new_df.iterrows():
    ...
    data = json.dumps({"timesheet": {"startDate": str(previous_date)},
                       "timesheetEntries": [
                           {"date": str(current_date), "projectId": project_id, "taskId": task_id, "startTime": "10:45",
                            "endTime": "18:45", "totalHours": 9, "comments": str(row.Description), "billable": "true",
    ...
    if (res.status_code == 200):
        print("Successfull for date: " + str(current_date))
    else:
        print("Unsuccessfull for date: " + str(current_date))
